	...
	while not terminated:
		...
		for i in range(n_ant):
			if i_episode <= 1500:
				...
				avail_actions_ind = np.nonzero(mask[i])[0]
				q_perference = q[i].cpu().detach().numpy()
				q_perference = q_perference[avail_actions_ind]
				p = np.exp(q_perference) / np.sum(np.exp(q_perference))
				a = np.random.choice(avail_actions_ind,p=p)
			action.append(a)
