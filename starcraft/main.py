...
env = StarCraft2Env(map_name='25m')
...
model_tar.load_state_dict(model.state_dict())  # todo: what is this doing?
optimizer = optim.RMSprop(model.parameters(), lr=0.0005)
# todo: figure out sys.argv
comm_flag = 1#float(sys.argv[1])
...
while i_episode<n_episode:
	...
	if i_episode > 100:
		epsilon -= 0.001
		if epsilon < 0.02:
			epsilon = 0.02  # todo: what is epsilon
	print("i_episode", i_episode)
	i_episode+=1
	env.reset()
	terminated = False
	obs = get_obs(env.get_obs(),n_ant)  # todo: what does the this encode
	adj = env.get_visibility_matrix()[:,0:n_ant]*comm_flag + np.eye(n_ant)  # todo: what is this
	...
	while not terminated:
		...
		action=[]
		q = model(torch.Tensor(np.array([obs])), torch.Tensor(np.array([adj])))[0]
	...
	for epoch in range(n_epoch):
		...
		target_q_values = (target_q_values - 9e15*(1 - torch.Tensor(Next_Mask))).max(dim = 2)[0]
